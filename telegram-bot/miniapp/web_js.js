...
function renderUnifiedHistory() {
  ...
  const pfByCode = {};
  (_me?.items||[]).forEach(it=>{ pfByCode[it.fund_code]=it; });
  const mktByCode = {};
  (_marketData?.signals||[]).forEach(s=>{ mktByCode[s.code]=s; });
  let summaryHtml = '';
  const fundCodes = Object.keys(fundStats).sort();
  if (fundCodes.length && assetF !== 'gold' && !codeF) {
    let totalNetInvested=0, totalUnrealPnl=0, totalRealizedPnl=0;
    const cards = fundCodes.map(c => {
      const s = fundStats[c];
      const pf = pfByCode[c];
      const currNav = pf?.current_nav || mktByCode[c]?.nav || 0;
      const netUnits = s.buyUnits - s.sellUnits;
      const netInvested = s.buyAmt - s.sellAmt; // remaining cost basis approx
      const mktVal = currNav > 0 && netUnits > 0 ? netUnits * currNav : 0;
      const unrealPnl = pf ? (pf.pnl_amount || (mktVal - (pf.avg_cost||0) * netUnits)) : (mktVal - Math.max(0, netInvested));
      const realizedPnl = s.sellAmt - (s.sellUnits / Math.max(s.buyUnits, 0.0001)) * s.buyAmt;
      totalNetInvested += s.buyAmt;
      totalUnrealPnl   += (mktVal > 0 ? unrealPnl : 0);
      totalRealizedPnl += realizedPnl;
      const holding = netUnits > 0.001;
      const pnlColor = unrealPnl >= 0 ? 'var(--buy)' : 'var(--sell)';
      const sig = mktByCode[c];
      const sigBadge = sig?.signal ? `<span style="font-size:9px;padding:1px 5px;border-radius:3px;background:${sig.signal==='buy'?'#4ade8022':sig.signal==='sell'?'#f8717122':'#facc1522'};color:${sig.signal==='buy'?'var(--buy)':sig.signal==='sell'?'var(--sell)':'#facc15'};border:1px solid currentColor">${sig.signal==='buy'?'MUA':sig.signal==='sell'?'BÁN':'HOLD'}</span>` : '';
      return `<div style="background:var(--bg3);border-radius:8px;padding:10px 12px;min-width:130px;flex:1">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
          <span style="font-family:var(--mono);font-size:12px;font-weight:700;color:var(--txt1)">${c}</span>
          ${sigBadge}
        </div>
        <div style="font-size:9px;color:var(--txt3);margin-bottom:6px">${s.txCount} giao dịch · ${holding?'Đang nắm':'Đã đóng'}</div>
        <div style="font-size:10px;color:var(--txt2)">Đã mua: <b style="color:var(--txt1);font-family:var(--mono)">${(s.buyAmt/1e6).toFixed(1)}M</b></div>
        ${holding && mktVal > 0 ? `<div style="font-size:10px;color:var(--txt2)">Giá trị hiện tại: <b style="font-family:var(--mono);color:var(--c0)">${(mktVal/1e6).toFixed(1)}M</b></div>
        <div style="font-size:11px;font-weight:700;color:${pnlColor};margin-top:4px">${unrealPnl>=0?'+':''}${(unrealPnl/1e6).toFixed(2)}M (${unrealPnl/Math.max(s.buyAmt,1)*100>=0?'+':''}${(unrealPnl/Math.max(s.buyAmt,1)*100).toFixed(1)}%)</div>` : ''}
        ${realizedPnl !== 0 ? `<div style="font-size:10px;color:${realizedPnl>=0?'var(--buy)':'var(--sell)'}">Thực hiện: ${realizedPnl>=0?'+':''}${(realizedPnl/1e3).toFixed(0)}k</div>` : ''}
      </div>`;
    });
    const totPnlC = totalUnrealPnl >= 0 ? 'var(--buy)' : 'var(--sell)';
    summaryHtml = `<div style="margin-bottom:12px">
      <div style="font-size:10px;color:var(--txt3);font-family:var(--mono);letter-spacing:.07em;margin-bottom:8px">TỔNG KẾT THEO QUỸ</div>
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:8px;margin-bottom:10px">${cards.join('')}</div>
      <div style="background:var(--bg3);border-radius:8px;padding:10px 14px;display:flex;gap:24px;flex-wrap:wrap">
        <div><div style="font-size:9px;color:var(--txt3)">Tổng đã mua</div><div style="font-family:var(--mono);font-size:13px;font-weight:700">${(totalNetInvested/1e6).toFixed(1)}M đ</div></div>
        <div><div style="font-size:9px;color:var(--txt3)">Lãi/lỗ chưa thực hiện</div><div style="font-family:var(--mono);font-size:13px;font-weight:700;color:${totPnlC}">${totalUnrealPnl>=0?'+':''}${(totalUnrealPnl/1e6).toFixed(2)}M đ</div></div>
        ${totalRealizedPnl !== 0 ? `<div><div style="font-size:9px;color:var(--txt3)">Lãi/lỗ đã thực hiện</div><div style="font-family:var(--mono);font-size:13px;font-weight:700;color:${totalRealizedPnl>=0?'var(--buy)':'var(--sell)'}">${totalRealizedPnl>=0?'+':''}${(totalRealizedPnl/1e3).toFixed(0)}k đ</div></div>` : ''}
      </div>
      <div style="font-size:10px;color:var(--txt3);margin-top:6px;margin-bottom:12px;border-bottom:1px solid var(--bdr);padding-bottom:12px">LỊCH SỬ GIAO DỊCH</div>
    </div>`;
  }
...
function _renderHistFundList() {
  ...
  const alertHtml = alerts.length ? `<div style="background:#facc1511;border:1px solid #facc1533;border-radius:6px;padding:6px 8px;margin-bottom:6px;font-size:10px">
    <span style="color:#facc15;margin-right:4px">⚠</span><b style="color:var(--txt1)">Cần chú ý:</b>
    ${alerts.map(c=>{
      const sv=sigs[c];
      const note=sv.rsi<32?`RSI ${sv.rsi?.toFixed(0)} (quá bán)`:sv.rsi>68?`RSI ${sv.rsi?.toFixed(0)} (quá mua)`:sv.score>=4?`Score +${sv.score} (mua mạnh)`:sv.score<=-4?`Score ${sv.score} (bán mạnh)`:'';
      return `<span onclick="_selectHistFund('${c}')" style="cursor:pointer;color:var(--c0);font-family:var(--mono)">${c}</span><span style="color:var(--txt3)"> ${note}</span>`;
    }).join(', ')}
  </div>` : '';
...
function _renderHistAnalysis(code) {
  ...
  // RSI divergence (simple: compare last local price low vs RSI low)
  let rsiDivergence = null;
  if (pts.length >= 30 && s) {
    const last30nav = navVals.slice(-30);
    const minIdx = last30nav.indexOf(Math.min(...last30nav));
    // If current RSI is higher than typical at price bottom (approx)
    if (minIdx < 20 && rsi > 40 && curr < lo52 * 1.05) {
      rsiDivergence = 'bullish'; // price near low but RSI recovered = bullish divergence
    }
  }
  const advHtml = pts.length >= 20 ? _mkSect('PHÂN TÍCH KỸ THUẬT NÂNG CAO', '🔬', `
    ...
    ${rsiDivergence === 'bullish' ? `<div style="background:#4ade8011;border:1px solid #4ade8033;border-radius:6px;padding:8px 12px;font-size:10px;color:var(--buy)">📊 Phân kỳ dương (Bullish Divergence): Giá đang ở vùng thấp nhưng RSI (${rsi.toFixed(0)}) đã hồi phục — tín hiệu đảo chiều tăng. Nhà giao dịch kỹ thuật coi đây là cơ hội mua.</div>` : ''}
  `) : '';
  ...
  panel.innerHTML = `<div style="padding:8px 0 20px">${headerHtml}${pnlHtml}${perfHtml}${riskHtml}${rangeHtml}${advHtml}${indHtml}${predHtml}${conclusionHtml}</div>`;
}
