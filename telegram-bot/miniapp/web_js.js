...
function renderPfAlloc() {
  ...
  const total=(items.reduce((s,h)=>s+(h.value||0),0))+goldVal;
  if (!total) return;
  const FT_COLOR={equity:'var(--buy)',bond:'#60a5fa',balanced:'#a78bfa',money_market:'#facc15'};
  const FT_LABEL={equity:'Cổ phiếu',bond:'Trái phiếu',balanced:'Cân bằng',money_market:'Tiền tệ'};
  // Group by fund type
  const byType={};
  for (const h of items) {
    const ft=_marketData?.[h.code]?.fund_type||'equity';
    byType[ft]=(byType[ft]||0)+(h.value||0);
  }
  if (goldVal>0) byType['gold']=goldVal;
  const segments=Object.entries(byType).map(([ft,val])=>({ft,val,pct:val/total*100}));
  segments.sort((a,b)=>b.val-a.val);
  const trackHtml=segments.map(seg=>{
    const c=seg.ft==='gold'?'#fbbf24':(FT_COLOR[seg.ft]||'var(--c0)');
    return `<div style="width:${seg.pct.toFixed(1)}%;background:${c};height:100%"></div>`;
  }).join('');
  const legendHtml=segments.map(seg=>{
    const c=seg.ft==='gold'?'#fbbf24':(FT_COLOR[seg.ft]||'var(--c0)');
    const lbl=seg.ft==='gold'?'Vàng':(FT_LABEL[seg.ft]||seg.ft);
    return `<span style="color:${c};font-size:10px;font-family:var(--mono)">${lbl} ${seg.pct.toFixed(0)}%</span>`;
  }).join('<span style="color:var(--txt3);margin:0 3px">·</span>');
...
async function _buildPfMiniChart(items) {
  if (!items || !items.length) return;
  const canvasEl = document.getElementById('pf-mini-chart'); if (!canvasEl) return;
  const results = (await Promise.all(items.map(h =>
    fetch(`/api/nav_history/${h.code}?limit=60`).then(r=>r.json())
      .then(d => ({ code:h.code, units:h.units, navs:(d.history||[]).slice().reverse() }))
      .catch(()=>null)
  ))).filter(Boolean);
  if (!results.length) return;
  // Build date union, sorted ascending, take last 30
  const allDates = [...new Set(results.flatMap(r=>r.navs.map(p=>p.date)))].sort().slice(-30);
  // For each fund, sorted ascending navs
  const navMapArr = {};
  ...
  window._pfMiniChartInst = new Chart(canvasEl.getContext('2d'), {
    ...
    options:{
      ...
      plugins:{legend:{display:false}, tooltip:{callbacks:{label:c=>`${(c.raw/1e6).toFixed(2)}M đ`}}},
      scales:{
        x:{display:false},
...
function renderPortfolio(me) {
  ...
  if (!pf.items.length) {
    ...
    // Portfolio T+2 aggregate insight
    const pfT2 = _pfT2Summary(pf.items);
    if (pfT2) {
      const t2c = pfT2.pct>=0.3?'var(--buy)':pfT2.pct<=-0.3?'var(--sell)':'var(--txt2)';
      const t2sign = pfT2.pct>=0?'+':'';
      html+=`<div style="background:rgba(0,229,255,.06);border:1px solid rgba(0,229,255,.2);border-radius:8px;padding:8px 12px;margin-bottom:8px;display:flex;align-items:center;justify-content:space-between">
        <span style="font-size:11px;color:var(--txt2)">Dự báo T+2 danh mục (${pfT2.count} quỹ)</span>
        <span style="font-family:var(--mono);font-size:13px;font-weight:600;color:${t2c}">${t2sign}${pfT2.pct.toFixed(2)}%</span>
      </div>`;
    }
    // Portfolio mini chart (30-day value curve)
    html+=`<div style="background:var(--bg2);border:1px solid var(--bdr);border-radius:8px;padding:10px 12px;margin-bottom:8px">
      <div style="font-size:10px;color:var(--txt3);margin-bottom:6px">Giá trị danh mục 30 ngày</div>
    ...
    // Portfolio Health Intelligence Panel
    const _pfTotal = pf.total_value || 0;
    const _pfSigs = _marketData || _signals || {};
    const _pfConc = pf.items.map(h => ({
      code: h.code,
      pct: _pfTotal > 0 ? h.value / _pfTotal * 100 : 0,
      rsi: _pfSigs[h.code]?.rsi,
      score: _pfSigs[h.code]?.score || 0,
      signal: h.signal,
      pnl_pct: h.pnl_pct || 0,
      chg30: _pfSigs[h.code]?.chg30
    }));
    // Weighted portfolio score (−6 to +6)
    const _pfWeightedScore = _pfConc.reduce((s, c) => s + c.score * (c.pct / 100), 0);
    const _pfScoreColor = _pfWeightedScore >= 2 ? 'var(--buy)' : _pfWeightedScore <= -2 ? 'var(--sell)' : '#facc15';
    const _pfScoreLabel = _pfWeightedScore >= 3 ? 'Xu hướng tích cực' : _pfWeightedScore >= 1 ? 'Nhẹ nghiêng mua' : _pfWeightedScore <= -3 ? 'Xu hướng tiêu cực' : _pfWeightedScore <= -1 ? 'Nhẹ nghiêng bán' : 'Trung lập';
    // Most concentrated fund
    const _maxConc = _pfConc.reduce((mx, c) => c.pct > mx.pct ? c : mx, {pct:0,code:''});
    // Most oversold (lowest RSI)
    const _oversoldList = _pfConc.filter(c => c.rsi != null).sort((a,b) => a.rsi - b.rsi);
    const _mostOversold = _oversoldList[0];
    // Worst 1M performer
    const _worst30 = _pfConc.filter(c => c.chg30 != null).sort((a,b) => a.chg30 - b.chg30)[0];
    const _pfAlerts = [];
    if (_maxConc.pct > 40) _pfAlerts.push(`<span style="color:#facc15">⚠ ${_maxConc.code} chiếm ${_maxConc.pct.toFixed(0)}% danh mục — rủi ro tập trung cao</span>`);
    if (_mostOversold && _mostOversold.rsi < 30) _pfAlerts.push(`<span style="color:var(--buy)">💡 ${_mostOversold.code} RSI ${_mostOversold.rsi.toFixed(0)} — cực kỳ quá bán, tiềm năng tăng mạnh</span>`);
    else if (_mostOversold && _mostOversold.rsi < 40) _pfAlerts.push(`<span style="color:var(--buy)">💡 ${_mostOversold.code} RSI ${_mostOversold.rsi.toFixed(0)} — vùng quá bán, cân nhắc tích lũy</span>`);
    if (_worst30 && _worst30.chg30 < -10) _pfAlerts.push(`<span style="color:var(--sell)">📉 ${_worst30.code} giảm ${_worst30.chg30.toFixed(1)}% trong 1 tháng — theo dõi chặt</span>`);
    // Concentration bars (top 4 funds)
    const _topConc = [..._pfConc].sort((a,b) => b.pct - a.pct).slice(0, 5);
    const _concBars = _topConc.map(c => {
      const barC = c.pct > 40 ? 'var(--sell)' : c.pct > 25 ? '#facc15' : 'var(--buy)';
      return `<div style="margin-bottom:5px">
        <div style="display:flex;justify-content:space-between;font-size:10px;margin-bottom:2px">
          <span style="font-family:var(--mono);color:var(--txt1)">${c.code}</span>
          <span style="color:${barC};font-family:var(--mono)">${c.pct.toFixed(1)}%</span>
        </div>
        <div style="height:5px;background:var(--bdr);border-radius:3px;overflow:hidden">
          <div style="height:100%;width:${Math.min(c.pct,100).toFixed(1)}%;background:${barC};border-radius:3px"></div>
        </div>
      </div>`;
    }).join('');
    html += `<div style="background:var(--bg2);border:1px solid var(--bdr);border-radius:10px;padding:12px 14px;margin-bottom:8px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
        <div style="font-size:10px;color:var(--txt3);font-family:var(--mono);letter-spacing:.06em">PORTFOLIO INTELLIGENCE</div>
        <div style="display:flex;align-items:center;gap:6px">
          <span style="font-size:10px;color:var(--txt3)">Score</span>
          <span style="font-family:var(--mono);font-size:14px;font-weight:700;color:${_pfScoreColor}">${_pfWeightedScore >= 0 ? '+' : ''}${_pfWeightedScore.toFixed(1)}</span>
          <span style="font-size:9px;color:${_pfScoreColor}">${_pfScoreLabel}</span>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;align-items:start">
        <div>
          <div style="font-size:9px;color:var(--txt3);margin-bottom:6px;font-family:var(--mono)">TỶ TRỌNG THEO QUỸ</div>
          ${_concBars}
        </div>
        <div>
          ${_pfAlerts.length ? `<div style="font-size:9px;color:var(--txt3);margin-bottom:6px;font-family:var(--mono)">CẢNH BÁO & GỢI Ý</div>
          <div style="display:flex;flex-direction:column;gap:8px;font-size:10px;line-height:1.5">${_pfAlerts.join('')}</div>` : `<div style="font-size:10px;color:var(--buy);line-height:1.6">✓ Danh mục ổn định — không có cảnh báo nào cần chú ý.</div>`}
        </div>
      </div>
    </div>`;
...
function renderResearchInline(d, el, canvasId) {
  ...
  html += _resIndicators(d);
  html += _t2PredHtml(d);
  if (hasHistory) html+=`<div style="padding:8px 14px 10px;height:300px;position:relative"><canvas id="${canvasId}"></canvas></div>`;
...
function _renderHistFundList() {
  ...
  const alertHtml = alerts.length ? `<div style="background:#facc1511;border:1px solid #facc1533;border-radius:6px;padding:5px 8px;margin-bottom:6px;font-size:10px;line-height:1.7">
    ...
    }).join('')}${alertsMore > 0 ? `<span style="font-size:9px;color:var(--txt3);padding:1px 4px">+${alertsMore} quỹ khác</span>` : ''}</div>
  </div>` : '';
  el.innerHTML = alertHtml + goldRow + funds.map(code => {
    ...
    const chg30 = s.chg30 ?? null;
    const chg30Html = chg30!=null ? `<span style="font-size:9px;color:${chg30>=0?'var(--buy)':'var(--sell)'};font-family:var(--mono)">${chg30>=0?'+':''}${chg30.toFixed(1)}% 1T</span>` : '';
    const sig = s.signal || '';
    ...
    // Score mini-bar (−6 to +6 range shown as 12-slot bar)
    const sc = s.score || 0;
    const scColor = sc >= 3 ? 'var(--buy)' : sc <= -3 ? 'var(--sell)' : '#facc15';
    const scBarW = Math.round(Math.abs(sc) / 6 * 20); // max 20px
    const scBarLeft = sc < 0; // grows left from center
    const scBar = `<div style="display:flex;align-items:center;gap:3px;flex-shrink:0" title="Score ${sc >= 0 ? '+' : ''}${sc}">
      <div style="width:20px;height:3px;background:var(--bg3);border-radius:2px;position:relative;overflow:hidden">
        <div style="position:absolute;${scBarLeft?'right':'left'}:${scBarLeft?0:0}px;width:${scBarW}px;height:100%;background:${scColor};border-radius:2px;${scBarLeft?'right:0':'left:0'}"></div>
      </div>
      <span style="font-size:9px;font-family:var(--mono);color:${scColor};min-width:16px">${sc >= 0 ? '+' : ''}${sc}</span>
    </div>`;
    // RSI dot
    const rsiDotC = (s.rsi != null) ? (s.rsi < 35 ? 'var(--buy)' : s.rsi > 65 ? 'var(--sell)' : 'var(--txt3)') : 'transparent';
    const rsiDot = s.rsi != null ? `<span style="width:6px;height:6px;background:${rsiDotC};border-radius:50%;display:inline-block;flex-shrink:0" title="RSI ${s.rsi?.toFixed(0)}"></span>` : '';
    return `<div class="hist-fund-row ${_histPageCode===code?'active':''}" onclick="_selectHistFund('${code}',this)">
      ...
      <div class="hist-fund-nav" style="display:flex;align-items:center;gap:5px;flex-wrap:wrap">${nav} ${chgHtml}${_sparklineSvg(code)}</div>
      <div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap">${sigHtml}${chg30Html}${rsiDot}${scBar}</div>
    </div>`;
  }).join('');
}
...
function _renderHistAnalysis(code) {
    ...
    pnlHtml = _mkSect('V\u1ecb TH\u1ebe \u0110ANG N\u1eafM GI\u1eef', '\u{1F4BC}', `
      ...
      ${(() => {
        ...
        if (pnlP < -5) {
          ...
          return `<div style="background:var(--sell)11;border:1px solid var(--sell)33;border-radius:8px;padding:8px 12px;margin-top:8px;font-size:10px;color:var(--txt2)"><b style="color:var(--sell)">Hòa vốn:</b> cần NAV tăng +${_toBreak.toFixed(1)}% (về ${fmt(Math.round(avgCost))}đ/CCQ).${_dcaHint}${_recovNote}</div>`;
        }
  ...
  const perfHtml = _mkSect('HIỆU SUẤT ĐẦU TƯ', '📈', `
    ...
    ${consistencyPct != null ? `<div style="background:var(--bg3);border-radius:8px;padding:8px 12px;margin-top:8px;display:flex;justify-content:space-between;align-items:center">
      <div>
        <div style="font-size:10px;color:var(--txt2)">Tỷ lệ tuần tăng</div>
        <div style="font-size:9px;color:var(--txt3)">NAV tuần sau > tuần trước · ${totalWeeks} tuần đã đo</div>
      </div>
      <div style="text-align:right">
        <div style="font-family:var(--mono);font-size:15px;font-weight:700;color:${consistencyC}">${consistencyPct}%</div>
        <div style="font-size:9px;color:${consistencyC}">${consistencyLabel}</div>
      </div>
    </div>` : ''}
...
`) : '';
  ...
  const rangeHtml = nav52.length > 10 ? _mkSect('V\u00d9NG GI\u00c1 52 TU\u1ea6N', '\u{1F4CF}', `
    <div style="background:var(--bg3);border-radius:8px;padding:12px 14px">
      <div style="display:flex;justify-content:space-between;font-size:10px;color:var(--txt3);margin-bottom:8px">
        <span style="display:inline-flex;flex-direction:column"><span>\u0110\u00e1y: <b style="color:var(--sell);font-family:var(--mono)">${fmt(Math.round(lo52))}</b></span><span style="font-size:9px;color:var(--txt3);font-family:var(--mono)">${_fmtD52(_lo52Pt?.date)}</span></span>
        <span>Hi\u1ec7n t\u1ea1i: <b style="color:var(--c0);font-family:var(--mono)">${fmt(Math.round(curr))}</b></span>
        <span style="display:inline-flex;flex-direction:column;align-items:flex-end"><span>\u0110\u1ec9nh: <b style="color:var(--buy);font-family:var(--mono)">${fmt(Math.round(hi52))}</b></span><span style="font-size:9px;color:var(--txt3);font-family:var(--mono);text-align:right">${_fmtD52(_hi52Pt?.date)}</span></span>
      </div>
  ...
  const advHtml = pts.length >= 20 ? _mkSect('PHÂN TÍCH KỸ THUẬT NÂNG CAO', '🔬', `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
      ...
      ${bbW ? `<div style="background:var(--bg3);border-radius:8px;padding:10px 12px">
        ...
        <div style="font-size:10px;color:var(--txt2);margin-bottom:6px">${bbW.expanding === true ? '↑ Đang mở rộng — biến động tăng' : bbW.expanding === false ? '↓ Đang thu hẹp — áp lực tích lũy' : ''}</div>
  ...
  const histPatternHtml = histPatternData ? (() => {
    ...
    return _mkSect('LỊCH SỬ TÍN HIỆU TƯƠNG TỰ', '📊', `
      <div style="font-size:10px;color:var(--txt3);margin-bottom:8px">Trong ${n} lần lịch sử có ${zone} — hiệu suất trung bình của quỹ trong các giai đoạn tiếp theo:</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px">
        <div style="background:var(--bg3);border-radius:8px;padding:10px 12px;text-align:center">
          <div style="font-size:9px;color:var(--txt3);margin-bottom:4px;font-family:var(--mono)">SAU 30 NGÀY</div>
          <div style="font-family:var(--mono);font-size:18px;font-weight:700;color:${avgC30}">${avg30>=0?'+':''}${avg30.toFixed(1)}%</div>
          <div style="font-size:9px;color:var(--txt3);margin-top:2px">trung bình</div>
          <div style="font-size:11px;color:${winC30};margin-top:5px;font-family:var(--mono)">${winRate30}% lần tăng</div>
        </div>
        <div style="background:var(--bg3);border-radius:8px;padding:10px 12px;text-align:center">
          <div style="font-size:9px;color:var(--txt3);margin-bottom:4px;font-family:var(--mono)">SAU 60 NGÀY</div>
          <div style="font-family:var(--mono);font-size:18px;font-weight:700;color:${avgC60}">${avg60>=0?'+':''}${avg60.toFixed(1)}%</div>
          <div style="font-size:9px;color:var(--txt3);margin-top:2px">trung bình</div>
          <div style="font-size:11px;color:${winC60};margin-top:5px;font-family:var(--mono)">${winRate60}% lần tăng</div>
        </div>
      </div>
      <div style="display:flex;gap:5px;margin-bottom:8px">
        <div style="background:var(--bg3);border-radius:6px;padding:5px 10px;font-size:10px;flex:1;text-align:center">
          <span style="color:var(--txt3);font-size:9px">Tốt nhất (30N)</span>
          <span style="font-family:var(--mono);color:var(--buy);margin-left:5px">${best30>=0?'+':''}${best30.toFixed(1)}%</span>
        </div>
        <div style="background:var(--bg3);border-radius:6px;padding:5px 10px;font-size:10px;flex:1;text-align:center">
          <span style="color:var(--txt3);font-size:9px">Xấu nhất (30N)</span>
          <span style="font-family:var(--mono);color:var(--sell);margin-left:5px">${worst30.toFixed(1)}%</span>
        </div>
      </div>
      <div style="background:${avg30>=0&&winRate30>=60?'#4ade8011':avg30<0&&winRate30<=40?'#f8717111':'var(--bg3)'};border:1px solid ${avg30>=0&&winRate30>=60?'#4ade8033':avg30<0&&winRate30<=40?'#f8717133':'var(--bdr)'};border-radius:6px;padding:7px 10px;font-size:10px;color:var(--txt2);line-height:1.5">${insight}</div>
      <div style="font-size:9px;color:var(--txt3);margin-top:6px;line-height:1.5">Dựa trên NAV lịch sử của chính quỹ này. Quá khứ không đảm bảo tương lai — bối cảnh thị trường thay đổi có thể tạo ra kết quả khác.</div>
    `);
  })() : '';
  ...
  const summaryLine = (() => {
    ...
    if (score >= 5 && cheapTech) return `Tín hiệu kỹ thuật rất tích cực: giá đang ở vùng rẻ (RSI ${rsi.toFixed(0)}, BB ${bb.toFixed(0)}%) và đà tăng đang hình thành.${_maCtx}${_rangeCtx}${histConf}`;
    if (score >= 3 && macd > 0)  return `Tín hiệu mua — đà tăng đang mạnh${cheapTech ? ', giá còn ở vùng hấp dẫn' : ''}.${_maCtx}${_fibCtx}${histConf}`;
    if (score <= -5 && expTech)  return `Tín hiệu kỹ thuật tiêu cực: giá đang ở vùng đắt (RSI ${rsi.toFixed(0)}, BB ${bb.toFixed(0)}%) và đà giảm đang hình thành.${_maCtx}${_rangeCtx}`;
    if (score <= -3 && macd < 0) return `Tín hiệu bán — đà giảm đang mạnh${expTech ? ', giá ở vùng cao' : ''}. Thận trọng khi mua thêm.${_maCtx}${_fibCtx}`;
    if (score === 0)             return `Tín hiệu trung lập. Các chỉ số kỹ thuật không nghiêng hẳn về chiều nào.${_maCtx}${histConf}`;
    return score > 0 ? `Tín hiệu nhẹ nghiêng về mua — chưa đủ điều kiện mua mạnh.${_maCtx}${_fibCtx}${histConf}` : `Tín hiệu nhẹ nghiêng về bán — chờ thêm xác nhận.${_maCtx}${_fibCtx}`;
  })();
  const strategyHtml = (() => {
  ...
  panel.innerHTML = `<div style="padding:8px 0 20px">${headerHtml}${pnlHtml}${perfHtml}${riskHtml}${rangeHtml}${advHtml}${indHtml}${histPatternHtml}${predHtml}${conclusionHtml}</div>`;
}
    ...
    try {
      ...
      if (!hist.length) {
        el.innerHTML=`<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;gap:12px;padding:32px;text-align:center">
          ...
          <div style="font-size:13px;font-weight:600;color:var(--txt1)">${code} — Chưa có lịch sử T+2</div>
          <div style="font-size:11px;color:var(--txt3);max-width:320px;line-height:1.7">Biểu đồ T+2 so sánh NAV dự báo vs thực tế. Dữ liệu tích lũy sau khi pipeline T+2 chạy đủ ngày trên Railway.<br>Trên môi trường local, dữ liệu T+2 chưa có.</div>
        </div>`;
