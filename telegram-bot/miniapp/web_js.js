...
function _renderHistAnalysis(code) {
  ...
  const advHtml = pts.length >= 20 ? _mkSect('PHÂN TÍCH KỸ THUẬT NÂNG CAO', '🔬', `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
      ${ma20val ? `<div style="background:var(--bg3);border-radius:8px;padding:10px 12px">
        ...
        ${maCrossSignal === 'golden' ? `<div style="color:var(--buy);font-size:11px;font-weight:700">✨ GOLDEN CROSS — MA20 vừa cắt lên MA50. Tín hiệu mua mạnh (rất hiếm).</div>` :
          ...
          maCrossSignal === 'above'  ? `<div style="color:var(--buy);font-size:10px">MA20 trên MA50 — xu hướng tăng trung hạn đang duy trì. Tốt.</div>` :
          maCrossSignal === 'below'  ? `<div style="color:var(--sell);font-size:10px">MA20 dưới MA50 — xu hướng giảm trung hạn. Cẩn thận.</div>` : ''}
        <div style="font-size:9px;color:var(--txt3);margin-top:5px;line-height:1.4">MA20/50: trung bình 20/50 ngày. NAV trên MA = xu hướng tăng.</div>
      </div>` : '<div></div>'}
      ${bbW ? `<div style="background:var(--bg3);border-radius:8px;padding:10px 12px">
        ...
        <div style="font-size:9px;color:var(--txt3);line-height:1.4">${bbW.curr < 3 ? 'Dải hẹp: thường xảy ra trước breakout — theo dõi sát.' : bbW.curr > 10 ? 'Dải rộng: biến động cao, rủi ro tăng theo cả hai chiều.' : 'Dải bình thường: biến động trong ngưỡng trung bình.'}</div>
      </div>` : ''}
    </div>
    ${fibLevels.length ? `<div style="background:var(--bg3);border-radius:8px;padding:10px 12px;margin-bottom:8px">
      ...
      <div style="font-size:9px;color:var(--txt3);margin-top:8px;line-height:1.4">Fibonacci: các mức hỗ trợ/kháng cự tự nhiên. Giá thường "bật" tại các mức 38.2%, 50%, 61.8%. Mức hiện tại (${pct52}% từ đáy) ${nearFib.length ? `gần Fib ${nearFib[0].f}% — vùng quan trọng.` : 'giữa các mức Fib.'}</div>
    </div>` : ''}
  ...
  const histPatternHtml = (() => {
    if (!s || pts.length < 120) return '';
    const apiRSI = s.rsi ?? 50;
    // Only show in extreme zones
    if (apiRSI >= 35 && apiRSI <= 65) return '';
    ...
    const avg30 = instances.reduce((a,x)=>a+x.ret30,0)/instances.length;
    const avg60 = instances.reduce((a,x)=>a+x.ret60,0)/instances.length;
    const best30 = Math.max(...instances.map(x=>x.ret30));
    const worst30 = Math.min(...instances.map(x=>x.ret30));
    const winRate30 = Math.round(n30pos/instances.length*100);
    const winRate60 = Math.round(n60pos/instances.length*100);
    ...
    const zone = apiRSI < 35
      ? `RSI < 35 (vùng quá bán)` : `RSI > 65 (vùng quá mua)`;
    ...
    return _mkSect('LỊCH SỬ TÍN HIỆU TƯƠNG TỰ', '📊', `
      <div style="font-size:10px;color:var(--txt3);margin-bottom:8px">Trong ${instances.length} lần lịch sử có ${zone} — hiệu suất ${instances.length > 1 ? 'trung bình' : ''} của quỹ trong các giai đoạn tiếp theo:</div>
      ...
      <div style="background:${avg30>=0&&winRate30>=60?'#4ade8011':avg30<=-2&&winRate30<=40?'#f8717111':'var(--bg3)'};border:1px solid ${avg30>=0&&winRate30>=60?'#4ade8033':avg30<=-2&&winRate30<=40?'#f8717133':'var(--bdr)'};border-radius:6px;padding:7px 10px;font-size:10px;color:var(--txt2);line-height:1.5">${insight}</div>
