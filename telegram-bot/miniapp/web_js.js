...
async function _buildPfMiniChart(items) {
  ...
  const results = (await Promise.all(items.map(h =>
    apiFetch(`/api/nav_history/${h.code}?limit=60`)
  ...
  if (!pf.items.length) {
    ...
    html+=`<div style="background:var(--bg2);border:1px solid var(--bdr);border-radius:8px;padding:10px 12px;margin-bottom:8px">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">
        ...
        <span id="pf-chart-daterange" style="font-size:9px;color:var(--txt3);font-family:var(--mono)"></span>
      </div>
...
function _renderHistAnalysis(code) {
  ...
  const riskHtml = pts.length > 30 ? _mkSect('R\u1ee6I RO & CH\u1ea4T L\u01af\u1ee2NG', '\u{1F6E1}\uFE0F', `
    ...
    ${_winRate ? '<div style="background:var(--bg3);border-radius:8px;padding:10px 12px;margin-top:8px">'
      + '<div style="font-size:10px;font-weight:600;color:var(--txt1);margin-bottom:6px">🏆 Tỷ lệ thắng/thua hàng ngày</div>'
      + '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px">'
      ...
      + (_winRate.gainPain != null
        ...
        : '')
      + '</div></div>' : ''}
    ${_volPctile ? '<div style="background:var(--bg3);border-radius:8px;padding:8px 12px;margin-top:8px;font-size:10px;color:var(--txt2);line-height:1.6">'
