...
function _renderHistAnalysis(code) {
  ...
  const summaryLine = (() => {
    ...
    if (score >= 5 && cheapTech) return `Tín hiệu kỹ thuật rất tích cực: giá đang ở vùng rẻ (RSI ${rsi.toFixed(0)}, BB ${bb.toFixed(0)}%) và đà tăng đang hình thành.${histConf}`;
    if (score >= 3 && macd > 0)  return `Tín hiệu mua — đà tăng đang mạnh${cheapTech ? ', giá còn ở vùng hấp dẫn' : ''}.${histConf}`;
    ...
    if (score === 0)             return `Tín hiệu trung lập. Các chỉ số kỹ thuật không nghiêng hẳn về chiều nào.${histConf}`;
    return score > 0 ? `Tín hiệu nhẹ nghiêng về mua — chưa đủ điều kiện mua mạnh.${histConf}` : `Tín hiệu nhẹ nghiêng về bán — chờ thêm xác nhận.`;
  })();
  const strategyHtml = (() => {
